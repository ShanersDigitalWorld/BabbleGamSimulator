...
import net.mcreator.bubblegumsimulator.procedures.AtlantisPlayerEntersDimensionmusicProcedure;
import net.mcreator.bubblegumsimulator.item.AtlantisItem;
...
public class AtlantisDimension extends BubbleGumSimulatorModElements.ModElement {
	@ObjectHolder("bubble_gum_simulator:atlantis")
	public static final ModDimension dimension = null;
	@ObjectHolder("bubble_gum_simulator:atlantis_portal")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("atlantis"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("bubble_gum_simulator:atlantis")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("bubble_gum_simulator:atlantis"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("bubble_gum_simulator:atlantis"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("bubble_gum_simulator:atlantisbio")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new AtlantisItem().setRegistryName("atlantis"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = AtlantisDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("atlantis_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "atlantis_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = AtlantisDimension.CustomPortalBlock.createPatternHelper(this.world, blockpos);
		...
		if (event.getTo() == type) {
			{
				...
				AtlantisPlayerEntersDimensionmusicProcedure.executeProcedure($_dependencies);
			}
		}
	}
