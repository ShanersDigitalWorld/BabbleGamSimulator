...
import net.mcreator.bubblegumsimulator.procedures.BeachPlayerEntersDimensionmusicProcedure;
import net.mcreator.bubblegumsimulator.item.BeachItem;
...
public class BeachDimension extends BubbleGumSimulatorModElements.ModElement {
	@ObjectHolder("bubble_gum_simulator:beach")
	public static final ModDimension dimension = null;
	@ObjectHolder("bubble_gum_simulator:beach_portal")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("beach"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("bubble_gum_simulator:beach")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("bubble_gum_simulator:beach"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("bubble_gum_simulator:beach"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("bubble_gum_simulator:beachbio")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new BeachItem().setRegistryName("beach"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = BeachDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("beach_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "beach_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = BeachDimension.CustomPortalBlock.createPatternHelper(this.world, blockpos);
		...
		if (event.getTo() == type) {
			{
				...
				BeachPlayerEntersDimensionmusicProcedure.executeProcedure($_dependencies);
			}
		}
	}
