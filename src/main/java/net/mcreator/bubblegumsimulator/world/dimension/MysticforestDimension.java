...
package net.mcreator.bubblegumsimulator.world.dimension;
...
import net.mcreator.bubblegumsimulator.procedures.MysticForestPlayerEntersDimensionmusicProcedure;
import net.mcreator.bubblegumsimulator.item.MysticforestItem;
import net.mcreator.bubblegumsimulator.block.SeagreenblockBlock;
import net.mcreator.bubblegumsimulator.block.PortalframeblockBlock;
import net.mcreator.bubblegumsimulator.block.DeeporangeblockBlock;
...
public class MysticforestDimension extends BubbleGumSimulatorModElements.ModElement {
	@ObjectHolder("bubble_gum_simulator:mysticforest")
	public static final ModDimension dimension = null;
	@ObjectHolder("bubble_gum_simulator:mysticforest_portal")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("mysticforest"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("bubble_gum_simulator:mysticforest")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("bubble_gum_simulator:mysticforest"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("bubble_gum_simulator:mysticforest"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("bubble_gum_simulator:mythicforestbio")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new MysticforestItem().setRegistryName("mysticforest"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("mysticforest_portal");
		}
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = MysticforestDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == PortalframeblockBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == PortalframeblockBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == PortalframeblockBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("mysticforest_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "mysticforest_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = MysticforestDimension.CustomPortalBlock.createPatternHelper(this.world,
						blockpos);
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag ? PortalframeblockBlock.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, PortalframeblockBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		if (event.getTo() == type) {
			{
				...
				MysticForestPlayerEntersDimensionmusicProcedure.executeProcedure($_dependencies);
			}
		}
	}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new EndGenerationSettings() {
				public BlockState getDefaultBlock() {
					return SeagreenblockBlock.block.getDefaultState();
				}

				public BlockState getDefaultFluid() {
					return DeeporangeblockBlock.block.getDefaultState();
				}
			});
