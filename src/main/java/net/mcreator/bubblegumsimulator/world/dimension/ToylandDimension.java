...
import net.mcreator.bubblegumsimulator.procedures.ToylandPlayerEntersDimensionmusicProcedure;
import net.mcreator.bubblegumsimulator.item.ToylandItem;
...
import net.mcreator.bubblegumsimulator.block.LimegreenblockBlock;
...
public class ToylandDimension extends BubbleGumSimulatorModElements.ModElement {
	@ObjectHolder("bubble_gum_simulator:toyland")
	public static final ModDimension dimension = null;
	@ObjectHolder("bubble_gum_simulator:toyland_portal")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("toyland"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("bubble_gum_simulator:toyland")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("bubble_gum_simulator:toyland"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("bubble_gum_simulator:toyland"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("bubble_gum_simulator:toylandbio")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new ToylandItem().setRegistryName("toyland"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("toyland_portal");
		}
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = ToylandDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("toyland_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "toyland_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = ToylandDimension.CustomPortalBlock.createPatternHelper(this.world, blockpos);
		...
		if (event.getTo() == type) {
			{
				...
				ToylandPlayerEntersDimensionmusicProcedure.executeProcedure($_dependencies);
			}
		}
	}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new EndGenerationSettings() {
				public BlockState getDefaultBlock() {
					return LimegreenblockBlock.block.getDefaultState();
				}

				public BlockState getDefaultFluid() {
					return RustblockBlock.block.getDefaultState();
				}
			});
